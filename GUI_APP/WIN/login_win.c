...
//宏定义
#define LOGIN_USERNAME	"123456"	//登录手机号 
#define LOGIN_PASSWORD	"abcdef"	//登录密码 

//变量定义
static lv_obj_t * login_win = NULL;		//登录窗口
...
{
	if(obj==username_ta||obj==password_ta)
	{
		if(event==LV_EVENT_RELEASED)//松手事件
		{
			lv_ta_set_cursor_type(username_ta,LV_CURSOR_LINE|LV_CURSOR_HIDDEN);//先隐藏光标
	...
	}else if(obj==login_btn)
	{
		...
		{
			const char * phone = lv_ta_get_text(username_ta);//获取输入的电话号码
			const char * password = lv_ta_get_text(password_ta);//获取输入的密码
			if((strcmp(phone,LOGIN_USERNAME)==0)&&(strcmp(password,LOGIN_PASSWORD)==0))
			{ 
				//验证成功
...
static void kb_create(lv_obj_t * bind_ta)
{
	...
	lv_kb_set_mode(kb,bind_ta==username_ta?LV_KB_MODE_NUM:LV_KB_MODE_TEXT);//如果绑定的是电话输入框,则弹出数字键盘,否则弹出文本键盘
...
//	lv_obj_align(logo_img,title_label,LV_ALIGN_OUT_BOTTOM_MID,0,20);//设置对齐方式
	//创建一个手机号输入框
	username_ta = lv_ta_create(login_win,NULL);
	lv_obj_set_size(username_ta,parent_width*0.7f,30);//设置大小
//	lv_obj_align(username_ta,logo_img,LV_ALIGN_OUT_BOTTOM_MID,0,20);//设置对齐方式
	lv_obj_align(username_ta,title_label,LV_ALIGN_OUT_BOTTOM_MID,0,70);//设置对齐方式
	lv_ta_set_text(username_ta,"");//清空默认的文本内容
	lv_ta_set_placeholder_text(username_ta,"Input username");//设置placeholder提示文本
	lv_ta_set_one_line(username_ta,true);//使能单行模式
	lv_obj_set_event_cb(username_ta,event_handler);//设置事件回调函数
	//创建一个密码输入框
	password_ta = lv_ta_create(login_win,username_ta);//从username_ta上进行拷贝
	lv_ta_set_pwd_mode(password_ta,true);//使能密码保护模式
	lv_obj_align(password_ta,username_ta,LV_ALIGN_OUT_BOTTOM_MID,0,10);
	lv_ta_set_cursor_type(password_ta,LV_CURSOR_LINE|LV_CURSOR_HIDDEN);//先隐藏光标
