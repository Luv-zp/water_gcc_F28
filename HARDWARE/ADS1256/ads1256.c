...
/*
*函数功能：初始化ADS1256需要用到的引脚以及其使用的通讯方式（SPI）
*输入参数：无
*返 回 值：无
*/
void ADS1256_Init(void)
{
	...
	AD_RST_HIGH();
	delay_us(100);
	ADS1256_Send8Bit(ADS1256_CMD_REST);
	delay_ms(10);
	ADS1256_WaitDRDY();                                //当ADS1256_DRDY为低时才能写寄存器
	ADS1256_Send8Bit(ADS1256_CMD_SYNC);                 //同步命令
	ADS1256_Send8Bit(ADS1256_CMD_WAKEUP);               //同步唤醒
	
	ADS1256_WaitDRDY();                                //当ADS1256_DRDY为低时才能写寄存器
	
	ADS1256_Send8Bit(ADS1256_CMD_WREG | ADS1256_STATUS);//连续写入4个寄存器
	ADS1256_Send8Bit(3);
	ADS1256_Send8Bit(0x04);                             //高位在前，使用内部校准，不使用缓存
	ADS1256_Send8Bit(ADS1256_MUXP_AIN2 | ADS1256_MUXN_AIN3);//端口输入A2为正，A3位负
	ADS1256_Send8Bit(ADS1256_GAIN_1);                   //放大倍数设置
	ADS1256_Send8Bit(ADS1256_DRATE_2000SPS);             //采集速度设置
	delay_us(100);
	
	ADS1256_WaitDRDY();                                 //当ADS1256_DRDY为低时才能写寄存器
	ADS1256_Send8Bit(ADS1256_CMD_SELFCAL);              //偏移和增益自动校准
...
static uint8_t ADS1256_ReadReg(uint8_t _RegID)
{
	...
	read = ADS1256_Recive8Bit();	/* 寄存器个数 - 1, 此处读1个寄存器 */
	AD_CS_HIGH();	/* SPI片选 = 1 */
...
static int32_t ADS1256_ReadData(void)
{
	...
	/* 读采样结果，3个字节，高字节在前 */
	read = SPI2_ReadWriteByte(0xff) << 16;
	read += (SPI2_ReadWriteByte(0xff) << 8);
