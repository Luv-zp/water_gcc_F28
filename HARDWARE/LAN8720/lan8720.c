...
//    PCF8574_WriteBit(ETH_RESET_IO,1);       //硬件复位
	HAL_GPIO_WritePin(GPIOI, GPIO_PIN_8, GPIO_PIN_RESET);  //PI8置0
    delay_xms(100);
//    PCF8574_WriteBit(ETH_RESET_IO,0);       //复位结束
	HAL_GPIO_WritePin(GPIOI, GPIO_PIN_8, GPIO_PIN_SET);  //PI8置1
    delay_xms(100);
    INTX_ENABLE();                          //开启所有中断  
...
void HAL_ETH_MspInit(ETH_HandleTypeDef *heth)
{
    ...
    HAL_NVIC_SetPriority(ETH_IRQn,5,0); //此处优先级必须小于configLIBRARY_MAX_SYSCALL_INTERRUPT_PRIORITY
