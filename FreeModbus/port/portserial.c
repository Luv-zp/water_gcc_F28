...
vMBPortSerialEnable( BOOL xRxEnable, BOOL xTxEnable )
{
	...
	if(xRxEnable == TRUE)	
		...
		__HAL_UART_DISABLE_IT( &UART1_Handler, UART_IT_RXNE );	//失能接收中断
	
	if(xTxEnable == TRUE)	
		__HAL_UART_ENABLE_IT( &UART1_Handler, UART_IT_TXE );	//使能发送中断	
	else	
		__HAL_UART_DISABLE_IT( &UART1_Handler, UART_IT_TXE );	//失能发送中断	
